...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                String buttonIDs = "btn" + i + j;
                int resIDs = getResources().getIdentifier(buttonIDs, "id", getPackageName());
                buttons[i][j] = findViewById(resIDs);
    ...
    private boolean checkForWin() {
        ...
        //Get all button posttopn
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                field[i][j] = buttons[i][j].getText().toString();
            }
        }
        //checking for all 3 columns
        for (int i = 0; i < 3; i++) {
